...
g_num = int(input('Enter your Redex|Base|Weight : '))
result = 0
...
if(number[:2]=='0b'):
    ...
    num_update = number[2:]
    print(num_update)
    result = sum(int(num_update[len(num_update)-i-1])*(g_num**i) for i in range(len(num_update)))
...
print(f'Base {g_num} Value : ',hex(result))
